"""Notebook-first report generation for AutoWiSP."""
...
class ReporterAgent:
    ...
    def generate(
        ...
        normalized_report = self._normalize_reliability_section(
            self._normalize_math_markdown(
                self._strip_outer_markdown_fence(self._strip_transient_preamble(report))
            ).strip()
        )
    ...
    def generate_with_review(
        ...
        normalized_report = self._normalize_reliability_section(
            self._normalize_math_markdown(
                self._strip_outer_markdown_fence(self._strip_transient_preamble(report_text))
            ).strip()
        )
        normalized_report = self._strip_appendices(normalized_report)
        normalized_report = self._ensure_inline_details(normalized_report, modeling_details, algorithm_details)
    ...
    @staticmethod
    def _strip_transient_preamble(text: str) -> str:
        stripped = text.lstrip("\ufeff").lstrip()
        stripped = re.sub(
            r"^(?:Thinking\.\.\.\s*\(\d+s elapsed\)\s*)+",
            "",
            stripped,
            flags=re.IGNORECASE,
        )
    ...
    def _build_footer(task_spec: dict, verification_results: dict, simulation_results: dict) -> str:
        ...
        parts = [
            "---",
            f"Generated by AutoWiSP on {datetime.now().isoformat(timespec='seconds')}",
